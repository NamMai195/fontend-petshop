...
import {
  ...
  increment, // Import increment for view count
...
const blogApiService = {
  ...
  /**
   * Fetches paginated blog posts from Firestore
  ...
  getPostById: async (postId) => {
    try {
      ...
      return {
        id: postSnap.id,
        ...postSnap.data(),
        createdAt: postSnap.data().createdAt?.toDate().toLocaleDateString(),
        updatedAt: postSnap.data().updatedAt?.toDate().toLocaleDateString()
      };
    } catch (error) {
  ...
  searchPosts: async (searchTerm) => {
    try {
      ...
      return snapshot.docs
        .filter(doc => {
          const data = doc.data();
          return data.title.toLowerCase().includes(searchTermLower) ||
